...
void HashTable::rehash() {
  ...
  Vector<Node*> new_table(new_size, nullptr);  // Используем конструктор

  for (size_t i = 0; i < table.size(); ++i) {
    Node* current = table[i];
    while (current) {
      Node* next_node = current->next;
      size_t new_idx = hash(current->key) % new_size;  // Исправлено
...
void HashTable::insert(const std::string& key, int line) {
  if (item_count > table.size() * 0.75) rehash();
...
Vector<int> HashTable::search(const std::string& key) const {
  ...
  while (current) {
    if (current->key == key) return current->lines;
    current = current->next;
  }
  return Vector<int>();
}
